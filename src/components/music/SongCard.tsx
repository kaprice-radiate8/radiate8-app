    ...
    <div className={featured ? "card p-5" : "py-2"}>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          <p className={`truncate font-serif text-plum ${featured ? "text-2xl leading-tight" : "text-lg"}`}>{song.title}</p>
          <p className="truncate text-sm text-plum-soft">{song.artist}</p>
        </div>
        <button
          type="button"
          onClick={() => toggle(song)}
          aria-label={playing ? `Pause ${song.title}` : `Play ${song.title}`}
          className={`flex shrink-0 items-center justify-center rounded-full bg-gold text-ivory shadow-md transition-transform duration-300 active:scale-95 ${featured ? "h-14 w-14" : "h-11 w-11"}`}
        >
      ...
      {featured && (
        <div className="mt-5 h-[3px] overflow-hidden rounded-full bg-line">
          <div className="h-full rounded-full bg-gold transition-[width] duration-300 ease-linear" style={{ width: `${showProgress ? progress * 100 : 0}%` }} />
        </div>
      )}
    </div>
  );
